import { Button, Card, CardActions, CardMedia, CardContent, Divider, Grid, Typography } from '@mui/material';
import navigateTo from './util/navigation';
import { useEffect, useState } from 'react';
import ReactGA from 'react-ga';
import MyProjects from './components/MyProjects';
import InterestingProjects from './components/InterestingProjects';
import githubApi from './api/github';

import './App.css';


function App() {
  const [me, setMe] = useState();

  useEffect(() => { 
    ReactGA.initialize('UA-64691528-3');
    ReactGA.pageview(window.location.pathname + window.location.search);
    
    const fetchMyGithub = async () => {
      const myGithub = await githubApi.getUser('jonathanhughes');
      setMe(myGithub);
    };
    fetchMyGithub();
  }, []);

  return (
    <div className="App" >
      <header className="App-header" style={{ backgroundImage: 'url("./background.jpg")', backgroundSize: 'auto', backgroundRepeat: 'repeat' }}>
        <Grid
          container
          spacing={2}
          direction="row"
          alignItems="stretch"
          justifyContent="center"
        >
          <Grid item md={3} lg={3} style={{ marginTop: 10 }}>
            <Grid
              container
              alignItems="center"
              justifyContent="center"
              style={{overflow: 'auto'}}
            >
              <Card>
                <CardMedia
                  component="img"
                  image={me?.avatar_url}
                  alt="profile"
                />
              </Card>
              <Card sx={{ marginTop: 5, marginInline: 3 }}>
                <CardContent>
                  <Typography gutterBottom variant="h4" component="div">
                    About me
                  </Typography>
                  <Typography variant="body1" color="text.secondary">
                    I'm a software engineer who loves innovating and tinkering.
                    I also love nature and being outdoors - running, cycling,
                    hiking, boating, kayaking, and traveling
                  </Typography>
                </CardContent>
              </Card>
              <Card sx={{ marginTop: 5, marginInline: 3 }}>
                <CardContent>
                  <Typography gutterBottom variant="h4" component="div">
                    <img width="153" height="28" loading="lazy" className="asurionLogo w-28 sm:w-32" src="data:image/svg+xml;base64," alt="Asurion Desktop Logo" />
                  </Typography>
                  <Typography variant="body1" color="text.secondary">
                    I work at Asurion. We're the largest privately held company in Nashville, TN.  Helping you protect, connect and enjoy all of your tech.
                  </Typography>
                  <Divider variant="middle" />
                  <CardActions>
                    <Button variant="contained" size="small" onClick={() => navigateTo('https://www.asurion.com')}>Learn More</Button>
                    <Button variant="contained" size="small" onClick={() => navigateTo('https://careers.asurion.com')}>Join our Team</Button>
                  </CardActions>
                </CardContent>
              </Card>
            </Grid>
          </Grid>
          <Grid item md={7} lg={7} >
            <InterestingProjects />
            <MyProjects />
          </Grid>
        </Grid>
      </header>
    </div>
  );

}

export default App;
